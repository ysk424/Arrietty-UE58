...
**状態:** C++ビルド、UEネイティブ座標テスト、Python回帰テスト、実機を使わない
UE上の離陸・終了を確認済みです。実機ではペダリングによる移動が確認され、
HMDの正面と走行方向を含む受け入れを継続中です。
詳細は[検証記録](docs/VALIDATION.md)を参照してください。
...
この瞬間のHMDの向きが自転車の正面になります。開始後に正面がずれていたら、
...
| R | 現在のコース方向へHMD正面とハンドル中央を合わせ直す。位置と経過時間を維持 |
